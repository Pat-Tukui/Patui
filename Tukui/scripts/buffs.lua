...
local function StyleBuffs(buttonName, index, debuff)
	...
	if icon and not _G[buttonName..index.."Panel"] then
		...
		count:SetFont(TukuiCF["media"].font, 12)
		...
		if ( buff.consolidated ) then
			...
			if ( (index > 1) and (mod(index, rowbuffs) == 1) ) then
				if ( index == rowbuffs+1 ) then
					buff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-10), TukuiDB.Scale(-75))
				else
					buff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-10), TukuiDB.Scale(-75))
				end
				aboveBuff = buff;
...
local function UpdateDebuffAnchors(buttonName, index)
	...
	if index == 1 then
		debuff:SetPoint("TOPRIGHT", UIParent, TukuiDB.Scale(-10), TukuiDB.Scale(-140))
	else
