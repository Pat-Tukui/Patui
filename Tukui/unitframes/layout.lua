...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		health.value:SetPoint("BOTTOMRIGHT", health, "BOTTOMRIGHT", TukuiDB.Scale(-1), 0)
		...
		power.value:SetPoint("BOTTOMLEFT", health, "BOTTOMLEFT", TukuiDB.Scale(1), 0)
		...
		if (unit == "player") then
			...
			if TukuiDB.myclass == "DRUID" then
                ...
                TukuiDB.CreatePanel(eclipseborder, 190, 8, "BOTTOM", self, "TOP", 0, TukuiDB.Scale(1))
                eclipseborder:SetFrameStrata("MEDIUM")
				
				local eclipseBar = CreateFrame('Frame', nil, self)
				eclipseBar:SetPoint("BOTTOMLEFT", self, "TOPLEFT", 0, TukuiDB.Scale(3))
				...
				eclipseBarText:SetPoint('TOPLEFT', health)
                eclipseBarText:SetPoint('TOPRIGHT', health)
			...
			if (TukuiDB.myclass == "WARLOCK" or TukuiDB.myclass == "PALADIN") then
				...
				TukuiDB.CreatePanel(shardborder, 190, 8, "BOTTOM", self, "TOP", 0, TukuiDB.Scale(1))
				shardborder:SetFrameStrata("MEDIUM")
	
				local bars = CreateFrame("Frame", nil, self)
				bars:SetPoint("BOTTOMLEFT", self, "TOPLEFT", 0, TukuiDB.Scale(3))
				bars:SetWidth(TukuiDB.Scale(182))
			...
			if TukuiDB.myclass == "DEATHKNIGHT" and db.runebar == true then
				...
				TukuiDB.CreatePanel(runeborder, 190, 8, "BOTTOM", self, "TOP", 0, TukuiDB.Scale(1))
				runeborder:SetFrameStrata("MEDIUM")
				
				local Runes = CreateFrame("Frame", nil, self)
				Runes:SetPoint("BOTTOMLEFT", self, "TOPLEFT", 0, TukuiDB.Scale(3))
				Runes:SetHeight(TukuiDB.Scale(4))
				Runes:SetWidth(TukuiDB.Scale(182))
				...
				for i = 1, 6 do
					...
					if (i == 1) then
						Runes[i]:SetPoint("BOTTOMLEFT", self, "TOPLEFT", 0, TukuiDB.Scale(3))
					else
			...
			if TukuiDB.myclass == "SHAMAN" and db.totemtimer == true then
				...
				TukuiDB.CreatePanel(totemborder, 190, 8, "BOTTOM", self, "TOP", 0, TukuiDB.Scale(1))
				...
				for i = 1, 4 do
					...
					if (i == 1) then
					   TotemBar[i]:SetPoint("BOTTOMLEFT", self, "TOPLEFT", 0, TukuiDB.Scale(3))
					else
		...
		if (db.unitcastbar == true) then
			...
			if (unit == "player") then
				castbar:SetPoint("TOPLEFT", Castbar, TukuiDB.Scale(2), TukuiDB.Scale(-2))
				castbar:SetPoint("BOTTOMRIGHT", Castbar, TukuiDB.Scale(-2), TukuiDB.Scale(2))
			elseif (unit == "target") then
				castbar:SetHeight(19)
				castbar:SetWidth(186)
				castbar:SetPoint("BOTTOM", health, "TOP", 0, TukuiDB.Scale(77))			
			end
...
tot:SetPoint("TOP", oUF_Tukz_target, "BOTTOM", 0,-8)
tot:SetSize(TukuiDB.Scale(186), TukuiDB.Scale(18))

-- pet
local pet = oUF:Spawn('pet', "oUF_Tukz_pet")
pet:SetPoint("TOP", oUF_Tukz_player, "BOTTOM", 0,-8)
...
-- player
local player = oUF:Spawn('player', "oUF_Tukz_player")
player:SetPoint("BOTTOMLEFT", TukuiActionBarBackground, "TOPLEFT", -185, 65)
player:SetSize(TukuiDB.Scale(186), TukuiDB.Scale(32))

-- target
local target = oUF:Spawn('target', "oUF_Tukz_target")
target:SetPoint("BOTTOMRIGHT", TukuiActionBarBackground, "TOPRIGHT", 190, 65)
target:SetSize(TukuiDB.Scale(186), TukuiDB.Scale(32))
