...
function TukuiDB.PP(p, obj)
	...
	local rightbg = RightSplitUnderPanel
	local leftbg = LeftSplitUnderPanel
	...
	if TukuiCF.actionbar.splitbar == true then
		if p == 9 then
	       obj:SetHeight(rightbg:GetHeight())
		   obj:SetPoint('TOP', rightbg)
		   obj:SetPoint('BOTTOM', rightbg)
	    elseif p == 10 then
	       obj:SetHeight(leftbg:GetHeight())
		   obj:SetPoint('TOP', leftbg)
		   obj:SetPoint('BOTTOM', leftbg)
		end
	end
end
...
do
	...
	TukuiDB.PostNamePosition = function(self)
		...
		if (self.Power.value:GetText() and UnitIsEnemy("player", "target") and TukuiCF["unitframes"].targetpowerpvponly == true) or (self.Power.value:GetText() and TukuiCF["unitframes"].targetpowerpvponly == false) then
			self.Name:SetPoint("TOPLEFT", self.Health, "TOPLEFT", TukuiDB.Scale(1), TukuiDB.Scale(-1))
		else
	...
	TukuiDB.PostUpdatePower = function(power, unit, min, max)
		...
		if self.Name then
			if unit == "target" then TukuiDB.PostNamePosition(self, health) end
		end
	end
